...
class OpenBankingBalanceQuerySerializer(serializers.Serializer):
    fintech_use_num = serializers.RegexField(regex=r"^[0-9A-Za-z]{4,24}$", max_length=24)
...
class OpenBankingTransactionQuerySerializer(serializers.Serializer):
    ...
    page = serializers.IntegerField(required=False, min_value=1, max_value=1000, default=1)
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (to_date - from_date) > timedelta(days=93):
            raise serializers.ValidationError({"range": "Maximum lookup window is 93 days."})
